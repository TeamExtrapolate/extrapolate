...
Z = data1[['level', 'state','difference']].groupby(['state','level'],as_index=False).mean()
...
for place in places_set:
    ...
    for index,row in filtered_Z.iterrows():
        ...
        text += str(round(float(row['difference']),2)) + "%"
    text_per_place[place] = text

# print (text_per_place)
# print (Z.loc[Z['place'] == 'Andaman and Nicobar Islands'])
# exit()
Y = data1[['state','difference']].groupby('state',as_index=False).mean().sort('difference',ascending=False)
...
for index in Y.index.values:
    city_count[Y['state'][index]] = Y['difference'][index]
    total += Y['difference'][index]
...
layout = graph_objs.Layout(
    title = "Percentage of Students placed w.r.t. enrollment",
    height=600,
    width=500,
    autosize=True,
    hovermode='closest',
...
print("uploading map")
print (tls.get_embed(py.plot(fig, filename='aicte_percentage_placed_map')))
